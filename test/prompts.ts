...
import prompt, { inject, prompt as _prompt, _injected } from "..";
...
test("prompts", (t) => {
	...
	types.forEach((p) => {
		...
		t.equal(
			typeof prompts[p],
			"function",
			`${prompts[p].name} is typeof function`
		);
	});
	...
	prompt({ type: "text", name: "a", message: "a message" }).then((foo) => {
		...
		prompt([
			...
			t.same(
				_injected,
				[],
				"leaves behind empty internal array when exhausted"
			);
			t.end();
		});
	});
